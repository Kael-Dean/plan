// Embed worksheet PDFs as full-page floating images at the end of each plan in the docx.
// Same approach as chapter 1's embed_pdf_in_docx.js but adapted for chapter 2 (10 plans).
...
async function main() {
  ...
  for (let i = 1; i <= PLAN_COUNT; i++) {
    ...
    const pdfPath = join(__dirname, "..", "แบบฝึกหัด", `ใบงานที่_${tn}_บทที่_๒_ใจหาย.pdf`);
    const pages = [];
  ...
  // 2. Load the chapter 2 docx
  const docxPath = join(__dirname, "..", "หน่วยการเรียนรู้ที่๒_ใจหาย_final5.docx");
  console.log(`\nLoading ${docxPath}`);
  ...
  // Use a small SAX walker similar to extract_ref
  ...
  // Build new body
  const parts = [];
  // Prelude (anything before first plan — should be empty)
  ...
  for (let i = 0; i < PLAN_COUNT; i++) {
    ...
    if (hasPageBreak) {
      // keep plan content without page break, then the page break (starts new page for worksheet)
      parts.push(planChunks.slice(0, -1).join(""));
      parts.push(lastChunk);
    } else {
      // last plan — no page break; add one before inserting worksheet
    ...
    for (let pg = 0; pg < wsPages.length; pg++) {
      ...
      // Page break between worksheet pages
