// Embed worksheet PDFs as full-page floating images after each plan in the chapter 4 docx.
// Same approach as chapter 2's embed_worksheets.mjs, adapted for chapter 4 (10 plans, กลัวทำไม).
...
async function main() {
  ...
  for (let i = 1; i <= PLAN_COUNT; i++) {
    ...
    const pdfPath = join(__dirname, "..", "แบบฝึกหัด", `ใบงานที่_${tn}_บทที่_๔_กลัวทำไม.pdf`);
    const pages = [];
