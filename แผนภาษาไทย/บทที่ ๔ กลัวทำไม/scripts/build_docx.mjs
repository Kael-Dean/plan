...
// === User customization: เก็บครบ ๑๐ แผน, normalize แผนละ ๑ ชั่วโมง, รวม ๑๐ ชั่วโมง ===
const KEEP_ORIGINAL = ["๑", "๒", "๓", "๔", "๕", "๖", "๗", "๘", "๙", "๑๐"];
const THAI_NUM = ["๑", "๒", "๓", "๔", "๕", "๖", "๗", "๘", "๙", "๑๐"];
const UNIT_TOTAL_HOURS = "๑๐";
const PLAN_HOURS = "๑";
const UNIT_TITLE = "หน่วยการเรียนรู้ที่ ๔ เรื่อง กลัวทำไม";
...
function buildSection1(section) {
  ...
  for (let bi = 0; bi < blocks.length; bi++) {
    ...
    for (let i = 0; i < block.lines.length; i++) {
      ...
      // keepNext on every line except the section's very last → whole section ๑
      // stays together as one block (heading moves down with its content).
      const isSectionLast =
        bi === blocks.length - 1 && i === block.lines.length - 1;
      if (block.header === "concept") {
        children.push(
          para([run(ln.text)], {
            indent: { left: IND(1.5) },
            keepNext: !isSectionLast,
            keepLines: true,
          }),
        );
      } else {
...
function buildGenericSection(section) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // keepNext on every line except the section's last → the whole section
    // (heading + all items) is one block Word pushes to the next page rather
    // than stranding the heading at the bottom. The last line stays unbound so
    // a page break is allowed between this section and the next.
    children.push(
...
function buildSection9() {
  ...
  // Paragraph inside the assessment table — keepNext so the heading + every row
  // cling together as one block. Combined with cantSplit on rows, Word pushes the
  // whole table to the next page instead of splitting it across a page boundary.
  ...
  function cell(lines, opts = {}) {
    const paras = lines.map((line) =>
      typeof line === "string" ? tp(line, opts) : line,
    );
    return new TableCell({
      children: paras,
      borders,
  ...
  const row1 = new TableRow({
    ...
    children: [
      cell([
        tp("ด้านความรู้ความเข้าใจ", { bold: true }),
        tp("1.  ออกเสียงคำศัพท์ ประโยคและบอกความหมาย"),
      ]),
      cell(["สังเกต"]),
      cell(["แบบบันทึกการสังเกต"]),
      cell([
        tp("นักเรียนผ่านเกณฑ์"),
        tp("โดยปฏิบัติกิจกรรมได้ถูกต้องร้อยละ 70"),
      ]),
    ],
  });
  ...
  const row2 = new TableRow({
    ...
    children: [
      cell([
        tp("ด้านทักษะ", { bold: true }),
        tp("ทักษะการฟัง การพูด การอ่าน การเขียน"),
      ]),
      cell([
        tp("ตรวจผลงาน"),
        tp("สังเกต"),
      ...
      cell([
        tp("แบบบันทึกการทำงาน"),
        tp("แบบบันทึกการสังเกต"),
      ]),
      cell([
        tp("นักเรียนผ่านเกณฑ์"),
        tp("โดยทำกิจกรรมถูกต้องอย่างน้อย ร้อยละ 70"),
      ]),
    ],
  });
  ...
  const row3 = new TableRow({
    ...
    children: [
      cell([
        tp("ด้านเจตคติ", { bold: true }),
        tp("1. ขยันใฝ่รู้ใฝ่เรียน"),
        tp("2. เชื่อมั่นในตนเอง /กล้าแสดงออก"),
        tp("3. มีเจตคติที่ดีต่อการเรียน"),
      ]),
      cell(["สังเกต"]),
      cell(["แบบบันทึกการสังเกต"]),
      cell([
        tp("นักเรียนผ่านเกณฑ์โดย"),
        tp("มีส่วนร่วมในกิจกรรม"),
        tp("และมีความสุขใน"),
        tp("การเรียนอย่างน้อย"),
        tp("ร้อยละ 80"),
      ]),
    ],
  });
...
async function build() {
  ...
  const doc = new Document({
    creator: "Chapter 4 Builder",
    title: "หน่วยการเรียนรู้ที่ ๔ กลัวทำไม",
    styles: {
      default: {
        document: {
  ...
  const outPath = join(__dirname, "..", "หน่วยการเรียนรู้ที่๔_กลัวทำไม.docx");
  writeFileSync(outPath, buf);
  console.log(`Wrote ${outPath} (${buf.length} bytes)`);
  console.log(`Plans included: ${kept.length}`);
}
